 ...
 * @date:   2015-12-24
 * @useage:
 * // https://github.com/HenrikJoreteg/Happy.js/blob/master/happy.js
 * // 初始化
 * var
 */

define('components/validator', function(require) {
    'use strict';
    // 依赖模块
    var $ = require('zepto');
    ...
    var fieldValidTypeHand=function($field,status,options){
        if($field.prop("type")=="radio" || $field.prop("type")=="checkbox"){
            ...
            if($fields.filter(":checked").length > 0){
                ...
                $fields.closest('.form-group').addClass('has-error')
            }
        }else{
    ...
    var fieldTooltip=(function(){
        ...
        function show(text){
            ...
            if(!instance){
                instance=true;
                $.dialog({ title: '提示', content: text }).on('dialog:hide', function(){ instance = false; });
            }
        }
    ...
    var validateField=function(event,options){
        var $field=$(this),
            ...
            errorTipFormat=$.fn.validator.errorTipFormat,//错误信息输出的格式化
    ...
    $.fn.validator=function(options){
        ...
        return this.validatorDestroy().each(function(event) {
    ...
    $.fn.validator.errorTipFormat=function(text){
        return '<div class="form-notice">'+text+'</div>';
    }

    // jquery 插件扩展
    //$.fn.validator = $.validator = Plugin;



    $(function(){ $('[data-validator]').validator()})
